...
const PEERS: readonly (readonly Cell[])[][] = Array.from({length: 9}, (_, r) =>
  Array.from({length: 9}, (__, c) =>
    [
      ...Array.from({length: 9}, (_, c2): Cell => [r, c2] as unknown as Cell).filter(([, c2]) => c2 !== c),
      ...Array.from({length: 9}, (_, r2): Cell => [r2, c] as unknown as Cell).filter(([r2]) => r2 !== r),
      ...Array.from({length: 9}, (_, k): Cell => [
        (r / 3 | 0) * 3 + (k / 3 | 0),
        (c / 3 | 0) * 3 + (k % 3),
      ] as unknown as Cell).filter(([pr, pc]) => !(pr === r && pc === c)),
    ].filter(([pr, pc], i, arr) =>
...
export function mrvBacktrack(board: BoardState): number[][] | null {
  ...
  for (let r = 0; r < 9; r++) {
    for (let c = 0; c < 9; c++) {
      ...
      cageCells.get(cid)!.push([r, c] as unknown as Cell);
  ...
  const cands: Set<number>[][] = Array.from({length: 9}, (_, r) =>
    Array.from({length: 9}, (__, c) => new Set(board.candidates[r][c])));

  return search(cands, cageOf, cageTotal, cageCells);
}
...
function search(
  ...
  for (const d of [...cands[r][c]].sort((a, b) => a - b)) {
    ...
    if (assign(newCands, r, c, d, cageOf, cageTotal, cageCells)) {
      const result = search(newCands, cageOf, cageTotal, cageCells);
