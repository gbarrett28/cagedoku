...
registered and enabled for this repo via `.Codex/settings.json`. Start
...
**Serena is a Codex plugin** (`serena@Codex-plugins-official`). Before using any serena tool,
...
that `serena@Codex-plugins-official` is enabled and restart the session before proceeding.
