...
class InpImage:
    ...
    def __init__(
        ...
        # Diagnostic attributes — overwritten with live values in the killer path;
        # default to 0/False when loaded from cache or when the classic path is taken.
        self.ink_density: float = 0.0
        self.fallback_used: bool = False
        self.total_sum: int = 0
        self.connectivity_score: int = 0
        ...
        self.ink_density = float(np.mean(warped_blk)) / 255.0
        self.info.border_x, self.info.border_y, self.connectivity_score = self._identify_borders(
    ...
    def _identify_borders(
        ...
        try:
            ...
            return initial_bx, initial_by, 0
        ...
        if best_score == n_heads:
            return best_bx, best_by, best_score
        ...
        for flip_box, flip_cell in ((True, False), (False, True), (True, True)):
            ...
            for gap in range(8):
                if (boundary_kind(gap) == BoundaryKind.BOX and flip_box) or (boundary_kind(gap) == BoundaryKind.CELL and flip_cell):
                    cx[:, gap] = 1.0 - cx[:, gap]
                    cy[gap, :] = 1.0 - cy[gap, :]
    ...
    def _build_cage_totals(
        ...
        if hiers_raw is not None:
            ...
            for _c, br, _ds in sorted(raw_nums, key=lambda ch: ch[1][0]):
                try:
                    num_chiers, _x, _y = split_num(br, warped_blk, subres)
                except ValueError:
