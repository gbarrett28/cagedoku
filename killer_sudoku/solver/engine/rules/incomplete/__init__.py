...
solve puzzles — but they have no ``as_hints`` implementation, so they cannot
...
1. Implement ``as_hints``.
2. Add ``@hintable_rule`` decorator.
3. Move the file to ``killer_sudoku/solver/engine/rules/``.
4. Add the import and __all__ entry to that package's ``__init__.py``.
5. Remove the import and list entry from this file.

Promoted rules (moved to rules/ and removed from incomplete_rules()):
  LinearElimination, DeltaConstraint, SumPairConstraint  (promoted earlier)
  CageIntersection, MustContain, PointingPairs, LockedCandidates,
  UnitPartitionFilter  (2026-04-02)
"""
